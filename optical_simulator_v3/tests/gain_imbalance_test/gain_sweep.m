...
config.config_tx.Lsymbs = 1000e3;                % Cantidad de simbolos transmitidos
...
ER_outer_db_v = linspace(18,38,10);
...
save(file,'ER_outer_db_v','theo_ber_v','config');
...
for idx_ER = 1:n_ER
    ...
    config.config_tx.ER_outer_db = ER_outer_db;

    parfor idx_ber = 1:n_ber

        osnr_db = osnr_db_v(idx_ber);
        config_aux = config;
        config_aux.config_ch.osnr_db = osnr_db;

        fprintf('- Running ER-%d/%d-ber-%d/%d ...\n', ...
            idx_ER, n_ER, idx_ber, n_ber)

        out_c{idx_ber} = optical_simulator_v3(config_aux);

    end

    name = sprintf('ER_outer_db_%2.2f',ER_outer_db);
