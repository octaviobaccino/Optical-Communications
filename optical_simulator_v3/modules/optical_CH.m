function [o_data_s] = optical_CH(i_ch_s, i_config_s)
    ...
    h_tx = root_raised_cosine(BR/2*1.35, fs_ch, rolloff, pulse_shaping_ntaps, 0);
    
    xup_h = filter(h_tx, 1, xup_h); 
    xup_v = filter(h_tx, 1, xup_v);
   
    % SOP rotation
    time = (0:length(xup_h)-1)'./fs_ch;
    
    yup_h = xup_h.*cos(2*pi*f_sop*time) + xup_v.*sin(2*pi*f_sop*time);
    yup_v = xup_h.*-sin(2*pi*f_sop*time) + xup_v.*cos(2*pi*f_sop*time);
    
    % Generacion de ruido
    snr_db = osnr_db - 10*log10(BR/12.5e9);
    snr = 10^(snr_db/10);
    Ps = var(yup_h(10e3:50e3))*2*N;
    Pn = Ps/snr;
