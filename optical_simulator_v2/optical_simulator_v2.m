...
function odata = optical_simulator_v2(i_config_s)
...
config.config_tx.Lsymbs = 400e3;
config.config_tx.rolloff = 0.1;
config.config_tx.pulse_shaping_ntaps = 81;
config.config_tx.pulse_shaping_type = 0;

% CH

config.config_ch.N = 2;
config.config_ch.osnr_db = 28;
config.config_ch.pulse_shaping_ntaps = 101;
config.config_ch.rolloff = 0.1;
config.config_ch.f_sop = 100e3;

% RX 

config.config_rx.down_phase = 0;
config.config_rx.tap_leak = 1e-2;
config.config_rx.target_agc = 0.3;
config.config_rx.eq_taps = 31;
config.config_rx.step_cma = 2^-10; % Con este step funciona el FSE con CMA (2^-10)
config.config_rx.step_dd = 2^-8;
...
valid = fix(0.7*length(ak_h)):length(ak_h)-100;
